...
Cytofpipe v1.3 can be used 1) to run standard cytometry data analysis for subset identification, 2) for 
comparison of groups of samples, and 3) to construct scaffold maps 
for visualizing complex relationships between samples. The methods underneath Cytofpipe v1.3 are based on 
publicly available R packages for flow/cytof data analysis
...
That will create a 'cytofpipe_1.3' masters folder that contains the code almost ready to use. You will just 
need to tell cytofpipe where you have downloaded the code. For that, open the cytofpipe_1.3/cytofpipe.pl perl 
script and change the variable CYTOFPIPE_HOME so that it points to the master folder:
...
Please make sure that these packages are installed, ad well as 'ini', 'hash', and OpenCyto (if you are 
planning to do automatic gating). Cytofpipe_v1.3 has been tested in R.3.4.2 with cytofkit 1.10.0 (available from bioconductor 3.5), scaffold 1.0, and citrus 0.08, and therefore we recommend that you install these specific versions to avoid potential clashes with older/newer versions. 
...
To run the pipeline in "clustering" mode with default parameters, just run the "cytofpipe.pl" perl script 
located in the masters cytofpipe_v1.3 folder:
...
`$ /path/to/cytofpipe_v1.3/cytofpipe.pl --citrus -i inputfiles --cond conditions.txt -o results -m 
markers.txt`
...
While we have a Cytofpipe version that runs in the UCL cluster using its queue system, we don't provide a 
version that will take advantage of the queue system in other clusters outside UCL. However, this is easily accomplished by using the 'asub' script developed by Heng Li at the Braod Institute (https://github.com/lh3/asub), and that we provide in the Cytofpipe_v1.3 masters folder for convenience.
