...
```
singularity pull --name cytofpipe.img shub://UCL-BLIC/singularity_recipes:cytofpipe
```
...
That will create a ‘cytofpipe-2.0’ masters folder that contains the code almost ready to use. 
You will just need to tell cytofpipe where you have downloaded the code. For that, open the 
cytofpipe-2.0/cytofpipe.pl perl script and change the variable CYTOFPIPE_HOME so
that it points to the master folder:

```
$ENV{CYTOFPIPE_HOME}="/path/where/your/have/cytofpipe-2.0"
```
...
located in the masters cytofpipe-2.0 folder:

`$ /path/to/cytofpipe-2.0/cytofpipe.pl -i inputfiles -o results -m markers.txt`
